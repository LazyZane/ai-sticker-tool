...
export function ImageToolSection() {
  ...
  const ratios = [
    ...
    { id: 'landscape', name: '横屏', ratio: 16/9 },
    { id: 'portrait', name: '竖屏', ratio: 9/16 },
    { id: 'photo', name: '照片', ratio: 4/3 },
    { id: 'free', name: '自由比例', ratio: null }
  ]
  ...
  const draw = useCallback(() => {
    if (!canvasRef.current || !imageRef.current.complete || imageRef.current.naturalWidth === 0) return
  ...
  const getHandleUnderMouse = (pos: { x: number, y: number }): Handle | null => {
    ...
    for (const handle of handles) {
      if (Math.abs(pos.x - handle.x) < handleSize && Math.abs(pos.y - handle.y) < handleSize) return handle.id
    }
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const r = Math.min(maxWidth / img.naturalWidth, maxHeight / img.naturalHeight)
      canvas.width = img.naturalWidth * r
      canvas.height = img.naturalHeight * r
      const initialCropWidth = canvas.width * 0.8
      const initialCropHeight = canvas.height * 0.8
      setCropArea({
        x: (canvas.width - initialCropWidth) / 2, y: (canvas.height - initialCropHeight) / 2,
        width: initialCropWidth, height: initialCropHeight,
      })
    }
    img.src = images[0].preview
  }, [images])

  useEffect(() => { draw() }, [cropArea, cropEnabled, draw, images])
  ...
  useEffect(() => {
    const handleWindowMouseMove = (e: MouseEvent) => {
      ...
      const ratio = ratios.find(r => r.id === selectedRatioId)?.ratio;
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold text-white mb-2">AI表情包图片处理工具</h1>
        <p className="text-slate-300">简单、高效，一键生成符合规范的表情包</p>
      </div>

      <div
        onDrop={handleDrop} onDragOver={handleDragOver}
        className="border-2 border-dashed border-slate-600 rounded-lg p-8 text-center hover:border-cyan-500 transition-colors duration-200 cursor-pointer mb-6"
        onClick={() => fileInputRef.current?.click()}
      >
        <Upload className="w-10 h-10 text-slate-400 mx-auto mb-3" />
        <h3 className="text-lg font-semibold text-white mb-1">点击或拖拽图片到此处上传</h3>
        <p className="text-slate-400 text-sm">（最多上传24张）</p>
      ...
      {images.length > 0 && (
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6 animate-fade-in">
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-white mb-3">已上传图片 ({images.length})</h3>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
              {images.map(image => (
                <div key={image.id} className="relative group">
                  <img src={image.preview} alt="preview" className="w-full h-20 object-cover rounded-md border-2 border-slate-700" />
            ...
            {cropEnabled && (
              <div className="grid lg:grid-cols-2 gap-6 animate-fade-in">
                  <div>
                    ...
                    <div className="bg-slate-900/50 rounded-lg flex justify-center items-center p-2 h-[320px]">
                      <canvas ref={canvasRef} onMouseDown={handleCanvasMouseDown} onMouseMove={handleCanvasMouseMove} />
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {ratios.map((ratio) => (
                        <button key={ratio.id} onClick={() => applyRatio(ratio.id)} className={`p-2 rounded text-sm transition-all duration-200 ${selectedRatioId === ratio.id ? 'bg-cyan-600 text-white' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-600/50'}`}>{ratio.name}</button>
                      ))}
                    </div>
